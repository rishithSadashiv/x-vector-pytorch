...
parser.add_argument('-training_filepath',type=str,default='meta/libri_training.txt')
parser.add_argument('-testing_filepath',type=str, default='meta/libri_testing.txt')
parser.add_argument('-validation_filepath',type=str, default='meta/libri_validation.txt')
...
def validation(dataloader_val,epoch):
    ...
    with torch.no_grad():
        ...
        model_save_path = os.path.join('save_model/libri', 'best_check_point_'+str(epoch)+'_'+str(mean_loss))
...
def test(dataloader_test):
    ...
    with torch.no_grad():
        ...
        mean_acc = accuracy_score(full_gts,full_preds)
        print('Total Test accuracy is {}'.format(mean_acc))
